...
client.createProduct = async function (name) {
    const product = {
        'name': name
    }
    const response = await fetch(`${baseUrl}/v1/products`, {
...
client.createPaymentLinkByProduct = async function(product){
    const resProduct  = await client.createProduct(product.name)
    const resPrice = await client.createPrice(product.price, 'usd', resProduct.id)
    const resPaymentLink = await client.createPaymentLink(resPrice.id, 1)
    return await resPaymentLink
}
