...
module.exports = function (fastify) {
    ...
    fastify.get(`${basePath}/api/order/commit`, async (request, reply) => {
        ...
        while (true) {
            ...
            await timers.setTimeout(3000)
            order = await datastoreClient.get('order', order.id)
            tryTime++
        }
        ...
        <body>
            ...
            <script>
                setTimeout(function(){
                    window.location = 'btsk://order?orderId=${order.orderId}'
                }, 2000);
            </script>
        </body></html>`)
    })
    ...
    fastify.get(`${basePath}/api/test`, async (request, reply) => {
        let order = await datastoreClient.get('order', 'ecb7d061-de80-47b8-8141-0c5ed4c02c2c')
        let user = await datastoreClient.get('user', order.username)
